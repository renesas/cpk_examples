...
static VOLATILE uint8_t s_rcache[W25QXX_SECTOR_SIZE];
...
#if TEST_QSPI_FLASH_EN_FULL_CHECK
...
uint32_t TestQSPIFlash(uint32_t start_addr, uint32_t size)
{
	uint8_t result;
...
#if TEST_QSPI_FLASH_EN_FULL_CHECK
	result = checkFullChipNoCache(W25QXX_SIZE / W25QXX_SECTOR_SIZE);
...
static uint8_t checkFullChipNoCache(uint32_t num_sector)
{
	...
	VOLATILE uint32_t *p32_r = (VOLATILE uint32_t *)W25QXX_MAP_START_ADDR;

	memset((void *)s_ram, 0, W25QXX_SECTOR_SIZE);
#if ENABLE_DIFF_CLK
	octaclk.source_clock = BSP_CLOCKS_SOURCE_CLOCK_PLL2Q;
#endif
#if ENABLE_CHIP_ERASE
	puts("Erase fullchip will take about 50 seconds, please wait");
	time_start = get_system_ms();
	W25QXX_EraseChip();
	W25QXX_WaitOperation(40000000);
	...
	for (j = 0; j < num_sector; j++) {
		...
		p32_e = (VOLATILE uint32_t *)(W25QXX_MAP_START_ADDR + j * W25QXX_SECTOR_SIZE);
		for (i = 0; i < (W25QXX_SECTOR_SIZE / 4); i++) {
			/* 计算 i 的有效位数 */
			bit_num = 0;
			t = i + j * W25QXX_SECTOR_SIZE / 4;
			while (t) {
				t >>= 0x01;
				bit_num++;
			}
			...
			p32_w[i] = ((mask & mask_src) | (i + j * W25QXX_SECTOR_SIZE / 4));
		}
		...
		for (t = 0; t < (W25QXX_SECTOR_SIZE / 256); t++) {
			err = W25QXX_Write((uint8_t *)p32_wcache, (uint8_t *)p32_wtoflash, 256, 0);
			if (err) {
				printf("%s ==> Write failed: %u\r\n", __FUNCTION__, err);
				printf("    t = %u\r\n", t);
				return 1;
			}
			err = W25QXX_WaitOperation(4000);
			if (err) {
				printf("%s ==> WaitOperation failed: %u\r\n", __FUNCTION__, err);
				printf("    t = %u\r\n", t);
				return 1;
			}
			p32_wcache = &p32_wcache[64];
			p32_wtoflash = &p32_wtoflash[64];
		}
		/* 计算写入速度 */
		time_end = get_system_us();
		if (time_end != time_start) {
			speed = (float)W25QXX_SECTOR_SIZE / (float)(time_end - time_start);
...
static void checkSpeedWrite(void)
{
	...
	float speed = 0.0f;
	VOLATILE uint32_t *p32_e = (VOLATILE uint32_t *)W25QXX_MAP_START_ADDR;
	VOLATILE uint32_t *p32_w = (VOLATILE uint32_t *)s_ram;
	VOLATILE uint32_t *p32_wtoflash = p32_e;
	...
	R_OSPI_B_Erase(OSPI_INSTANCE.p_ctrl, (uint8_t *)p32_e, W25QXX_SECTOR_SIZE);
	W25QXX_WaitOperation(4000);

	time_start = get_system_us();
	for (t = 0; t < (W25QXX_SECTOR_SIZE / 64); t++) {
		W25QXX_Write((uint8_t *)p32_w, (uint8_t *)p32_wtoflash, 64, 0);
		W25QXX_WaitOperation(4000);
		p32_w = &p32_w[16];
...
static void checkSpeedRead(void)
{
	...
	VOLATILE uint8_t *p8_flash = (VOLATILE uint8_t *)W25QXX_MAP_START_ADDR;
	...
	VOLATILE uint16_t *p16_flash = (VOLATILE uint16_t *)W25QXX_MAP_START_ADDR;
	...
	VOLATILE uint32_t *p32_flash = (VOLATILE uint32_t *)W25QXX_MAP_START_ADDR;
	...
	VOLATILE uint64_t *p64_flash = (VOLATILE uint64_t *)W25QXX_MAP_START_ADDR;
	...
	if (time_start != time_end) {
		...
		printf("Read from QSPI Flash to DTCM                  with  8bit width, ");
	...
	if (time_start != time_end) {
		speed = (float)CACHE_SIZE / (float)(time_end - time_start);
		speed = speed * 1000000 / 1024 / 1024;
		printf("Read from QSPI Flash to DTCM                  with 16bit width, ");
		printf("using cycle: %llu, ", tick_end - tick_start);
		printf("using time: %llu us, ", time_end - time_start);
		printf("speed: %.2f MB/s\r\n", speed);
	}
	...
	if (time_start != time_end) {
		speed = (float)CACHE_SIZE / (float)(time_end - time_start);
		speed = speed * 1000000 / 1024 / 1024;
		printf("Read from QSPI Flash to DTCM                  with 32bit width, ");
		printf("using cycle: %llu, ", tick_end - tick_start);
		printf("using time: %llu us, ", time_end - time_start);
		printf("speed: %.2f MB/s\r\n", speed);
	}
	...
	if (time_start != time_end) {
		speed = (float)CACHE_SIZE / (float)(time_end - time_start);
		speed = speed * 1000000 / 1024 / 1024;
		printf("Read from QSPI Flash to DTCM                  with 64bit width, ");
		printf("using cycle: %llu, ", tick_end - tick_start);
		printf("using time: %llu us, ", time_end - time_start);
		printf("speed: %.2f MB/s\r\n", speed);
	}
	...
	if (time_start != time_end) {
		speed = (float)CACHE_SIZE / (float)(time_end - time_start);
		speed = speed * 1000000 / 1024 / 1024;
		printf("Read from QSPI Flash to RAM(cacheable area)   with  8bit width, ");
		printf("using cycle: %llu, ", tick_end - tick_start);
		printf("using time: %llu us, ", time_end - time_start);
		printf("speed: %.2f MB/s\r\n", speed);
	}
	...
	if (time_start != time_end) {
		speed = (float)CACHE_SIZE / (float)(time_end - time_start);
		speed = speed * 1000000 / 1024 / 1024;
		printf("Read from QSPI Flash to RAM(cacheable area)   with 16bit width, ");
		printf("using cycle: %llu, ", tick_end - tick_start);
		printf("using time: %llu us, ", time_end - time_start);
		printf("speed: %.2f MB/s\r\n", speed);
	}
	...
	if (time_start != time_end) {
		speed = (float)CACHE_SIZE / (float)(time_end - time_start);
		speed = speed * 1000000 / 1024 / 1024;
		printf("Read from QSPI Flash to RAM(cacheable area)   with 32bit width, ");
		printf("using cycle: %llu, ", tick_end - tick_start);
		printf("using time: %llu us, ", time_end - time_start);
		printf("speed: %.2f MB/s\r\n", speed);
	}
	...
	if (time_start != time_end) {
		speed = (float)CACHE_SIZE / (float)(time_end - time_start);
		speed = speed * 1000000 / 1024 / 1024;
		printf("Read from QSPI Flash to RAM(cacheable area)   with 64bit width, ");
		printf("using cycle: %llu, ", tick_end - tick_start);
		printf("using time: %llu us, ", time_end - time_start);
		printf("speed: %.2f MB/s\r\n", speed);
	}
	...
	if (time_start != time_end) {
		speed = (float)CACHE_SIZE / (float)(time_end - time_start);
		speed = speed * 1000000 / 1024 / 1024;
		printf("Read from QSPI Flash to RAM(uncacheable area) with  8bit width, ");
		printf("using cycle: %llu, ", tick_end - tick_start);
		printf("using time: %llu us, ", time_end - time_start);
		printf("speed: %.2f MB/s\r\n", speed);
	}
	...
	if (time_start != time_end) {
		speed = (float)CACHE_SIZE / (float)(time_end - time_start);
		speed = speed * 1000000 / 1024 / 1024;
		printf("Read from QSPI Flash to RAM(uncacheable area) with 16bit width, ");
		printf("using cycle: %llu, ", tick_end - tick_start);
		printf("using time: %llu us, ", time_end - time_start);
		printf("speed: %.2f MB/s\r\n", speed);
	}
	...
	if (time_start != time_end) {
		speed = (float)CACHE_SIZE / (float)(time_end - time_start);
		speed = speed * 1000000 / 1024 / 1024;
		printf("Read from QSPI Flash to RAM(uncacheable area) with 32bit width, ");
		printf("using cycle: %llu, ", tick_end - tick_start);
		printf("using time: %llu us, ", time_end - time_start);
		printf("speed: %.2f MB/s\r\n", speed);
	}
	...
	if (time_start != time_end) {
		speed = (float)CACHE_SIZE / (float)(time_end - time_start);
		speed = speed * 1000000 / 1024 / 1024;
		printf("Read from QSPI Flash to RAM(uncacheable area) with 64bit width, ");
		printf("using cycle: %llu, ", tick_end - tick_start);
		printf("using time: %llu us, ", time_end - time_start);
		printf("speed: %.2f MB/s\r\n", speed);
	}
