...
uint32_t LCD_PortDrawBitmap(LCD_Device *device, uint16_t x, uint16_t y, uint16_t length, uint16_t width, const uint8_t *bitmap)
{
    ...
    case COLOR_FORMAT_RGB888:
        p32_bg = &p32_bg[y * (device->length * device->length_dummy) + x];
        for (i = 0; i < width; i++) {
            memcpy(p32_bg, p32_bitmap, length * 4);
    ...
    case COLOR_FORMAT_RGB565:
        p16_bg = &p16_bg[y * (device->length + device->length_dummy) + x];
        for (i = 0; i < width; i++) {
            memcpy(p16_bg, p16_bitmap, length * 2);
