...
uint32_t IIC_ReadReg(uint32_t slave, uint16_t reg_addr, uint8_t addr_width, uint8_t *val, uint8_t val_width)
{
	...
	if (addr_width == 8) {
		...
		err = R_IIC_MASTER_Write(IIC0_MASTER.p_ctrl, wcache, 1, true);
	}
	else {
	...
	IIC_WAIT_TX_DONE(s_iic0_master_flag);
	if (val_width == 8) {
		err = R_IIC_MASTER_Read(IIC0_MASTER.p_ctrl, rcache, 1, false);
		IIC_WAIT_RX_DONE(s_iic0_master_flag);
		val[0] = rcache[0];
	}
	else {
		err = R_IIC_MASTER_Read(IIC0_MASTER.p_ctrl, rcache, 2, false);
		IIC_WAIT_RX_DONE(s_iic0_master_flag);
		val[0] = rcache[0];
		val[1] = rcache[1];
	}
...
uint32_t IIC_WriteMemory(uint32_t slave, uint16_t mem_addr, uint8_t addr_width, const uint8_t *wdata, uint16_t wlen)
{
	...
	if (addr_width == 8) {
		...
		if (wlen > 127) {
			...
			err = R_IIC_MASTER_Write(IIC0_MASTER.p_ctrl, wcache, 128, true);
		}
		else {
			memcpy(&wcache[1], wdata, wlen);
			err = R_IIC_MASTER_Write(IIC0_MASTER.p_ctrl, wcache, wlen + 1, true);
		}
	}
	else {
		wcache[0] = (mem_addr >> 8) & 0xFF;
		...
		if (wlen > 126) {
			...
			err = R_IIC_MASTER_Write(IIC0_MASTER.p_ctrl, wcache, wlen + 2, true);
		}
	}
	UNLIKE_RETURN(err, 0, "R_IIC_MASTER_Write failed: 0x%X", err);
	IIC_WAIT_TX_DONE(s_iic0_master_flag);
...
uint32_t IIC_WriteReg(uint32_t slave, uint16_t reg_addr, uint8_t addr_width, uint16_t val, uint8_t val_width)
{
	...
	if (addr_width == 8) {
		...
		if (val_width == 8) {
			...
			wcache[2] = (val >> 8) & 0xFF;
			wcache[3] = val & 0xFF;
			err = R_IIC_MASTER_Write(IIC0_MASTER.p_ctrl, wcache, 4, false);
		}
	}
	UNLIKE_RETURN(err, 0, "R_IIC_MASTER_Write failed: 0x%X", err);
	IIC_WAIT_TX_DONE(s_iic0_master_flag);
