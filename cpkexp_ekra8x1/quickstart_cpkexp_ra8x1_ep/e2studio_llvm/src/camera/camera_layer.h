...
#define FACE_DETECTION_DONE     (0x01 << 2)
...
extern uint8_t *bsp_camera_capture_image(void);

extern void new_frame(void);

extern void bsp_camera_capture_start(void);

extern uint8_t *bsp_get_camera_rot_buf(void);

void rotate_rgb565_90(const uint16_t *src, uint16_t *dst, int src_w, int src_h);
