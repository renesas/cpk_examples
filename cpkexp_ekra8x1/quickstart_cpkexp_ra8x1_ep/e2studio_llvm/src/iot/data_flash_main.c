...
#define FLASH_DF_BLOCK_1			0x27000040U
#define FLASH_DF_BLOCK_2			0x27000080U
#define FLASH_DF_BLOCK_3			0x270000C0U
#define FLASH_DATA_BLOCK_SIZE	(1024)
#define KEY_SIZE_IN_BYTES		64
#define BLOCK_ERASE_SIZE			32

#define FLASH_EVENT_ERASE_DONE	0x01
#define FLASH_EVENT_WRITE_DONE	(0x01 << 1)
...
static bool s_flash_erase_done = false;
static bool s_flash_write_done = false;
...
static char g_new_api_key_str[KEY_SIZE_IN_BYTES + 8];
...
bool_t initialise_data_flash(void)
{
    ...
    R_FLASH_HP_Open(&g_flash1_ctrl, &g_flash1_cfg);
    R_FLASH_HP_StatusGet(&g_flash1_ctrl, &status);
	if (status == FLASH_STATUS_BUSY) {
		LOG_I(__FUNCTION__, "FLASH_STATUS_BUSY");
	}
	else {
		LOG_I(__FUNCTION__, "FLASH_STATUS_IDLE");
	}

	LOG_D(__FUNCTION__, "Read api key from Data Flash");
	memcpy(&s_hp_new_flash_data, (void *)FLASH_DF_BLOCK_0, sizeof(s_hp_new_flash_data));
	LOG_D(__FUNCTION__, "id block: %s", s_hp_new_flash_data.id_block);
    for (i = 0; i < 4; i++) {
    		LOG_D(__FUNCTION__, "table[%u].key_valid: %u", i, s_hp_new_flash_data.table[i].key_valid);
    		LOG_D(__FUNCTION__, "table[%u].length_of_record_bytes: %u", i, s_hp_new_flash_data.table[i].length_of_record_bytes);
    		if ((s_hp_new_flash_data.table[i].length_of_record_bytes > 0) && (s_hp_new_flash_data.table[i].length_of_record_bytes < 65)) {
    			LOG_D(__FUNCTION__, "table[%u].data: %s", i, s_hp_new_flash_data.table[i].data);
    		}
    		else {
    			s_hp_new_flash_data.table[i].length_of_record_bytes = 0;
...
bool_t get_key_data(e_hp_data_flash_iot_keys_t id, char_t *active_key)
{
    ...
    if (res == true) {
    		if (s_hp_new_flash_data.table[i].length_of_record_bytes > 64) {
    			s_hp_new_flash_data.table[i].key_valid = false;
    			res = false;
    		}
    		else {
    			memcpy(active_key, s_hp_new_flash_data.table[i].data, s_hp_new_flash_data.table[i].length_of_record_bytes);
    		}
    }
...
bool_t update_key_data(e_hp_data_flash_iot_keys_t id)
{
    ...
    while (1) {
        ...
        if ((count--) == 0) {
            ...
            print_to_console("数据刷新失败，以下是可能原因：\r\n");
            print_to_console("\t1. API key 找不到\r\n");
            print_to_console("\t2. API key 因非法被服务器拒绝\r\n");
            print_to_console("\t3. 网络连接不存在\r\n");
            print_to_console("您可以按任意键进入 API key 输入流程");
            print_to_console("或按空格键返回菜单\r\n\r\n");
        }
    }
    ...
    if (true == res) {
        switch (id) {
		case IOT_KEY_CURRENCY:
			res = input_new_key("输入一个有效的 API key 然后按回车键保存它\r\n或者按空格返回菜单.\r\n", false);
			s_hp_new_flash_data.table[0].key_valid = true;
			s_hp_new_flash_data.table[0].length_of_record_bytes = strlen(g_new_api_key_str);
			memcpy(s_hp_new_flash_data.table[0].data, g_new_api_key_str, s_hp_new_flash_data.table[0].length_of_record_bytes);
			LOG_D(__FUNCTION__, "API key length: %u", s_hp_new_flash_data.table[0].length_of_record_bytes);

			if (true == res) {
				if (true == update_stored_keys()) {
					print_to_console("API key 已保存至 Data Flash.\r\n");
				}
			}
			break;
		case IOT_KEY_WEATHER:
			...
			s_hp_new_flash_data.table[1].length_of_record_bytes = strlen(g_new_api_key_str);
			memcpy(s_hp_new_flash_data.table[1].data, g_new_api_key_str, s_hp_new_flash_data.table[1].length_of_record_bytes);
			LOG_D(__FUNCTION__, "API key length: %u", s_hp_new_flash_data.table[1].length_of_record_bytes);
...
bool_t update_stored_keys (void)
{
    ...
    LOG_D(__FUNCTION__, "Erase Data Flash");
	err = R_FLASH_HP_Erase(g_flash1.p_ctrl, FLASH_DF_BLOCK_0, 2);
	if (err != FSP_SUCCESS) {
		LOG_E(__FUNCTION__, "Flash erase failed");
		return false;
	}
	R_BSP_SoftwareDelay(1, BSP_DELAY_UNITS_MILLISECONDS);

	LOG_D(__FUNCTION__, "Write Data Flash");
	err = R_FLASH_HP_Write(&g_flash1_ctrl, (const uint32_t)&s_hp_new_flash_data, FLASH_DF_BLOCK_0, sizeof(s_hp_new_flash_data));
	if (err != FSP_SUCCESS) {
		LOG_E(__FUNCTION__, "Flash write failed");
		return false;
	}
	R_BSP_SoftwareDelay(1, BSP_DELAY_UNITS_MILLISECONDS);
    ...
    if (err == FSP_SUCCESS)
    {
        ...
        if (FLASH_STATUS_IDLE == status)
        {
            ...
            if (strstr(stored_iot->id_block, s_hp_new_flash_data.id_block) == NULL)
            {
                ...
                err    = R_FLASH_HP_Erase(&g_flash1_ctrl, FLASH_DF_BLOCK_1, BLOCK_ERASE_SIZE);
...
// use Simulated keys
            ...
            {
                memcpy(&s_hp_new_flash_data, (void *)(unsigned long)FLASH_DF_BLOCK_1, sizeof(s_hp_new_flash_data));
...
{
	LOG_D(__FUNCTION__, "Enter");

	LOG_D(__FUNCTION__, "p_args->event: %d", p_args->event);
	if (p_args->event == FLASH_EVENT_NOT_BLANK) {}
		...
		s_flash_erase_done = true;
	}
	else if (p_args->event == FLASH_EVENT_WRITE_COMPLETE) {
		s_flash_write_done = true;
	}
	else {}

	LOG_D(__FUNCTION__, "Exit");
}
