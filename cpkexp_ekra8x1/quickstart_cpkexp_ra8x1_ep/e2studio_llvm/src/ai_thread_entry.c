...
#if ENABLE_DEBUG_OUT
static char s_print_buffer[128];
#endif
 ...
 * Description  : Whether to predict or classify the images collected by the camera is determined based on
 *                do_detection and do_classification
...
void ai_thread_entry (void * pvParameters)
{
    ...
    while (1) {
        bsp_camera_rgb565_to_rgb888(bsp_camera_out_buffer[g_rgb_buffer], bsp_camera_out_buffer888, BSP_CAM_WIDTH, BSP_CAM_HEIGHT);
        vTaskDelay(10);

        /* Both do_detection and do_classification are defined in commom_init.c */
