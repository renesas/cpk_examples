 ...
 * Description  : Board monitoring thread
 *                  - Check the core temperature every 10 ticks
 *                  - Check the event STATUS_UPDATE_INTENSE_INFO set by button_irq10_callback()
 *                  - Check the event STATUS_UPDATE_FREQ_INFO set by button_irq11_callback()
...
void board_mon_thread_entry (void * pvParameters)
{
    ...
    while (1)
    {
        /* "STATUS_UPDATE_KIS_INFO" 仅在 menu_kis.c -> kis_display_menu() 设置 */
        s_ux_bits = xEventGroupWaitBits(g_update_console_event, STATUS_UPDATE_KIS_INFO, pdFALSE, pdTRUE, 1);

        /* Check the core temperature every 10 ticks, if it changes, Set the event "STATUS_UPDATE_TEMP_INFO" for g_update_console_event */
        ...
        /* "STATUS_UPDATE_INTENSE_INFO" only set by button_irq10_callback() */
        ...
        /* "STATUS_UPDATE_FREQ_INFO" only set by button_irq11_callback() */
