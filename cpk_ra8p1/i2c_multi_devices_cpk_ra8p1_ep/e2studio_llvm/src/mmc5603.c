...
#define IIC_READ_CHECK(reg, val)	{ err = MMC5603_ReadReg(reg, val); UNLIKE_RETURN(err, 0, "Read failed: 0x%u", err); }
#define IIC_WRITE_CHECK(reg, val)	{ err = MMC5603_WriteReg(reg, val); UNLIKE_RETURN(err, 0, "Write failed: 0x%u", err); }
...
uint32_t MMC5603_GetXYZ(int32_t *x, int32_t *y, int32_t *z)
{
	...
	err = MMC5603_ReadMem(0x00, rcache, 9);
