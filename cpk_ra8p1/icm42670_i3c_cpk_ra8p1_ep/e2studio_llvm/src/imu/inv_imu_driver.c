...
#ifndef __INV_IMU_DEBUG
#define __INV_IMU_DEBUG	1
#endif
...
#define INV_LOGD(msg, ...)	LOG_D(TAG, msg, ##__VA_ARGS__)
#define INV_LOGW(msg, ...)	LOG_W(TAG, msg, ##__VA_ARGS__)
#define INV_LOGE(msg, ...)	LOG_E(TAG, msg, ##__VA_ARGS__)

#ifndef REPORT_THRESHOULD
#define REPORT_THRESHOULD	100
#endif

static int64_t s_call_cnt;
static int64_t s_time_start;
static int64_t s_time_end;
static int64_t s_time_sum;
                 ...
                 void (*sensor_event_cb)(inv_imu_sensor_event_t *event))
{
...
#if __INV_IMU_DEBUG
	uint8_t reg = 0;
	serif->read_reg(NULL, 0x75, &reg, 1);
	LOG_D(TAG, "WHO AM I: 0x%X", reg);
#endif
	...
	INV_LOGD("configure_serial_interface: %d", status);

	/* Reset device */
	status |= inv_imu_device_reset(s);
	INV_LOGD("inv_imu_device_reset: %d", status);

	/* Initialize hardware */
	status |= init_hardware_from_ui(s);
	INV_LOGD("init_hardware_from_ui: %d", status);
...
int inv_imu_device_reset(inv_imu_device_t *s)
{
	...
	inv_imu_sleep_us(2000);
	if (s->transport.serif.serif_type == UI_I3C) {
		I3C_CheckHotjoin();
	}
#if __INV_IMU_DEBUG
	LOG_D(TAG, "Try to read WHO_AM_I after soft reset");
	inv_imu_read_reg(s, WHO_AM_I, 1, &data);
	LOG_D(TAG, "WHO AM I: 0x%X", data);
#endif

	/* Restore slew-rate configuration */
	INV_LOGD("Restore slew-rate configuration");
	...
	INV_LOGD("Re-configure serial interface since it was reset");
	status |= configure_serial_interface(s);
...
#if __INV_IMU_DEBUG
	s_call_cnt++;
	s_time_start = get_system_us();
#endif
	status |= inv_imu_read_reg(s, FIFO_DATA, packet_size * packet_count, s->fifo_data);
#if __INV_IMU_DEBUG
	s_time_end = get_system_us();
	s_time_sum += (s_time_end - s_time_start);
	if (s_call_cnt == REPORT_THRESHOULD) {
		s_call_cnt = 0;
		LOG_D(TAG, "Average time of reading FIFO: %u us", (uint32_t)(s_time_sum / REPORT_THRESHOULD));
		s_time_sum = 0;
	}
#endif
...
static int configure_serial_interface(inv_imu_device_t *s)
{
	...
	} else if (s->transport.serif.serif_type == UI_I3C) {
		/* Use I3C SDR. HDR-DDR can be enabled later after SDR bring-up is stable. */
		status |= inv_imu_read_reg(s, INTF_CONFIG1, 1, &value);
		INV_LOGD("INTF_CONFIG1: 0x%X", value);
		if (value != 0x4D) {
			value = 0x4D;
			status |= inv_imu_write_reg(s, INTF_CONFIG1, 1, &value);
		}
