...
/**
 * @def		__ST7796U_INTERFACE
 * @brief	Select ST7796U interface
 * @details	Options
 * 			ST7796U_INTERFACE_MIPI
 * 			ST7796U_INTERFACE_4SPI
 * 				Due to the use of MISO in the PCB design to control RCX,
 * 				So even if the module is set to the 4SPI mode, it can't read back data
 * 			ST7796U_INTERFACE_RGB
 * 				It still need SPI interface to config screen
 */
...
/**
 * @def		ST7796U_4SPI_EN_CACHE
 * @brief	Whether to use caching
 * @note	Only verified in it set 1
 */
...
#elif __ST7796U_INTERFACE == ST7796U_INTERFACE_MIPI

#define ST7796U_GLCDC_INSTANCE  g_lcdc
#define ST7796U_GLCDC_CALLBACK  GLCDC_Callback
#define ST7796U_MIPI_INSTANCE   g_mipi_dsi0
#define ST7796U_MIPI_CALLBACK   MIPI_DSI_Callback
...
/**
 * @brief	Draw bitmap
 * @param	device A lcd device pointer
 * @param	x      X start position
 * @param	y      Y start position
 * @param	length Length of X-axis
 * @param	width  Length of Y-axis
 * @param	bitmap Bitmap pointer, must be RGB565 format and don't swap the high and low bytes
 * @retval	0: Success
...
uint32_t LCD_PortDrawBitmap(LCD_Device *device, uint16_t x, uint16_t y, uint16_t length, uint16_t width, const uint8_t *bitmap)
{
    ...
    case COLOR_FORMAT_RGB888:
        ...
        for (i = 0; i < width; i++) {
            ...
            p32_bg = &p32_bg[device->length + device->length_dummy];
    ...
    case COLOR_FORMAT_RGB565:
        ...
        for (i = 0; i < width; i++) {
            ...
            p16_bg = &p16_bg[device->length + device->length_dummy];
	...
	uint32_t index = x + y * device->length;
    ...
    R_SCI_B_SPI_Write(ST7796U_SPI_INSTANCE.p_ctrl, &p16_cache[y * device->length], width * device->length * 2, SPI_BIT_WIDTH_8_BITS);
#else
    uint16_t color;
...
/**
 * @brief	Draw point
 * @note	Only validate in RGB565 mode and SPI interface
 * @param	device A lcd device pointer
 ...
 * @param	length Length of X-axis
 * @param	width  Length of Y-axis
 * @param	color  Color value, only RGB565 format
 * @retval	0: Success or length or width is zero
 * 			1: x or y out of range
 */
uint32_t LCD_PortFillRectangle(LCD_Device *device, uint16_t x, uint16_t y, uint16_t length, uint16_t width, uint32_t color)
{
    ...
    case COLOR_FORMAT_ARGB8888:
    case COLOR_FORMAT_RGB888:
        p32_bg = &p32_bg[y * (device->length + device->length_dummy)];
        for (i = 0; i < width; i++) {
    ...
    case COLOR_FORMAT_ARGB4444:
    case COLOR_FORMAT_RGB565:
        p16_bg = &p16_bg[y * (device->length + device->length_dummy)];
        for (i = 0; i < width; i++) {
            for (j = x; j < (x + length); j++) {
    ...
    R_BSP_PinWrite(ST7796U_PIN_SPI_CS, BSP_IO_LEVEL_LOW);
    if (device->color_format == COLOR_FORMAT_RGB565) {
        R_SCI_B_SPI_Write(ST7796U_SPI_INSTANCE.p_ctrl, &p16_cache[y * device->length], row_num * device->length * 2, SPI_BIT_WIDTH_8_BITS);
    }
    else {
        R_SCI_B_SPI_Write(ST7796U_SPI_INSTANCE.p_ctrl, &p32_cache[y * device->length], row_num * device->length * 4, SPI_BIT_WIDTH_8_BITS);
    }
...
uint32_t LCD_PortFillScreen(LCD_Device *device, uint32_t color)
{
    ...
    if ((device->orientation == SCREEN_ORIENTATION_HORIZONTAL) || (device->orientation == SCREEN_ORIENTATION_HORIZONTAL_FLIP)) {
    	row_start = 0x31;
    	row_end = device->width + 0x31;
    	column_start = 0;
    	column_end = device->length;
    }
    else {
    	row_start = 0;
    	row_end = device->width;
    	column_start = 0x31;
    	column_end = device->length + 0x31;
    }
    ...
    if (device->color_format == COLOR_FORMAT_RGB565) {
        ...
        data_write[0] = (uint8_t)(color >> 8);
        data_write[1] = (uint8_t)color;
		for (i = 0; i < (device->width * device->length); i++) {
			R_SCI_B_SPI_Write(ST7796U_SPI_INSTANCE.p_ctrl, data_write, 2, SPI_BIT_WIDTH_8_BITS);
			waitTransferDone(DEFAULT_TIMEOUT);
		}
		R_BSP_PinWrite(ST7796U_PIN_SPI_CS, BSP_IO_LEVEL_HIGH);
...
/**
 * @brief	Init a lcd device
 * @note	When __ST7796U_INTERFACE is ST7796U_INTERFACE_4SPI,
 * 			must set device->orientation = SCREEN_ORIENTATION_VERTICAL first.
 * 			Only this direction has been verified.
 * @param	device A lcd device pointer
 * @retval	Always return 0
 */
uint32_t LCD_PortInit(LCD_Device *device)
{
    ...
    R_GLCDC_Open(ST7796U_GLCDC_INSTANCE.p_ctrl, ST7796U_GLCDC_INSTANCE.p_cfg);
    msg.channel = 0;
    ...
    for (i = 0; i < len_init_table; i++) {
        ...
        s_mipi_tx_done = 0;
        err = R_MIPI_DSI_Command(ST7796U_MIPI_INSTANCE.p_ctrl, &msg);
        if (err != FSP_SUCCESS) {
        #if __ST7796U_DEBUG
    ...
    R_GLCDC_Start(ST7796U_GLCDC_INSTANCE.p_ctrl);
#elif __ST7796U_INTERFACE == ST7796U_INTERFACE_4SPI
...
/**
 * @brief	Set backlight
 * @note	In fact, it is achieved through the backlight chip AW9364.
 * 			The SY7796U module does not have the function of adjusting the backlight by itself.
 * @bug		When it is called multiple times, the brightness of the backlight will be incorrect.
 * @param	device A lcd device point, can accept NULL because it is not actually used.
 * @param	bl     Range 0-16
 * @retval	Always return 0
 */
uint32_t LCD_PortSetBacklight(LCD_Device *device, uint8_t bl)
{
...
#if __ST7796U_DEBUG
	LOG_D(__FUNCTION__, "Original bl: %u", bl);
#endif
	...
	if (bl == 0) {
		R_BSP_PinAccessEnable();
		R_BSP_PinWrite(ST7796U_PIN_BACKLIGHT, BSP_IO_LEVEL_LOW);
		R_BSP_PinAccessDisable();
	}
	else {
		need_send = 17 - bl;
	#if __ST7796U_DEBUG
		LOG_D(__FUNCTION__, "need_send: %u", need_send);
	#endif

		R_BSP_PinAccessEnable();
		R_BSP_PinWrite(ST7796U_PIN_BACKLIGHT, BSP_IO_LEVEL_LOW);
		R_BSP_SoftwareDelay(30, BSP_DELAY_UNITS_MICROSECONDS);
		R_BSP_PinWrite(ST7796U_PIN_BACKLIGHT, BSP_IO_LEVEL_HIGH);
		R_BSP_SoftwareDelay(30, BSP_DELAY_UNITS_MICROSECONDS);
		for (uint8_t i = 1; i < need_send; i++) {
			R_BSP_PinWrite(ST7796U_PIN_BACKLIGHT, BSP_IO_LEVEL_LOW);
 ...
 * @note	Not yet verified. Don't call this function
 * @param	device      A lcd device pointer
...
/*============================ IMPLEMENTATION ================================*/
#if __ST7796U_INTERFACE == ST7796U_INTERFACE_MIPI
void ST7796U_GLCDC_CALLBACK(display_callback_args_t *p_args)
{
...
void ST7796U_MIPI_CALLBACK(mipi_dsi_callback_args_t *p_args)
{
