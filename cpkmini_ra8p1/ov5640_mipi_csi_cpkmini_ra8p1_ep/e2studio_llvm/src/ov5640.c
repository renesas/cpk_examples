...
#define VIN_INSTANCE			g_vin
#define VIN_CALLBACK			VIN_Callback
#define MIPI_CSI_CALLBACK		MIPI_CSI_Callback
#define OV5640_PIN_PWR_ON		BSP_IO_PORT_07_PIN_04
#define OV5640_PIN_RESET		BSP_IO_PORT_04_PIN_11
#elif OV5640_USING_INTERFACE == OV5640_INTERFACE_DVP
#define CEU_INSTANCE	g_ceu
#define CEU_CALLBACK	CEU_Callback

#define OV5640_PIN_PWR_ON		BSP_IO_PORT_07_PIN_10
#define OV5640_PIN_RESET		BSP_IO_PORT_07_PIN_09
#else
#error "Unsupport OV5640 interface"
#endif

#define iicWrite(reg, val)		IIC_Write16bReg8bVal(OV5640_IIC_ADDR, reg, val)
#define iicRead(reg, val)		IIC_Read16bReg8bVal(OV5640_IIC_ADDR, reg, val)
...
#if OV5640_USING_INTERFACE == OV5640_INTERFACE_CSI
static const struct OV5640Reg sc_init_table[] = {
	/* Power down */
	{0x3008, 0x42},
	/* System setting. */
	{0x3103, 0x13},
...
static void hardReset(void)
{
	...
	R_BSP_PinWrite(OV5640_PIN_RESET, BSP_IO_LEVEL_LOW);
	R_BSP_PinWrite(OV5640_PIN_PWR_ON, BSP_IO_LEVEL_HIGH);
	R_BSP_PinAccessDisable();
}

static void setPower(uint8_t power)
{
	R_BSP_PinAccessEnable();
	if (power) {}
	else {
		...
		R_BSP_PinWrite(OV5640_PIN_PWR_ON, BSP_IO_LEVEL_LOW);
		R_BSP_SoftwareDelay(50, BSP_DELAY_UNITS_MILLISECONDS);
		R_BSP_PinWrite(OV5640_PIN_RESET, BSP_IO_LEVEL_HIGH);
		R_BSP_SoftwareDelay(20, BSP_DELAY_UNITS_MILLISECONDS);
	}
	R_BSP_PinAccessDisable();
}
...
uint32_t OV5640_Init(uint16_t length, uint16_t width)
{
	...
	uint16_t init_len;
	fsp_err_t err;
#if __OV5640_DEBUG || (OV5640_USING_INTERFACE == OV5640_INTERFACE_CSI)
	uint8_t val;
#endif
...
#if OV5640_USING_INTERFACE == OV5640_INTERFACE_DVP
    ...
    OV5640_SetOutputSize(length, width);
    R_BSP_SoftwareDelay(100, BSP_DELAY_UNITS_MILLISECONDS);
...
uint32_t OV5640_StartCapture(uint8_t *image_buffer)
{
	...
	ceu_instance_ctrl_t *p_ctrl = (ceu_instance_ctrl_t *)CEU_INSTANCE.p_ctrl;

	if (p_ctrl->open == 0) {
		R_CEU_Open(CEU_INSTANCE.p_ctrl, CEU_INSTANCE.p_cfg);
	}
	err = R_CEU_CaptureStart(CEU_INSTANCE.p_ctrl, image_buffer);
#else
	vin_instance_ctrl_t *p_ctrl = (vin_instance_ctrl_t *)VIN_INSTANCE.p_ctrl;
	IIC_WRITE_CHECK(0x4202, 0x0F);
	if (p_ctrl->open == 0) {
		R_VIN_Open(VIN_INSTANCE.p_ctrl, VIN_INSTANCE.p_cfg);
	}
	R_VIN_CaptureStart(VIN_INSTANCE.p_ctrl, image_buffer);
