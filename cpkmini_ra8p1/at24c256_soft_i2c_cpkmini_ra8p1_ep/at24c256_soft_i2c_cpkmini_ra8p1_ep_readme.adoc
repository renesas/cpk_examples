...
在 e^2^ studio 中导入工程，生成并编译，或直接烧录本文件夹下的 at24c256_soft_i2c.hex。打开一个串口调试工具，波特率 2000000，数据位 8bit，停止位 1bit，无奇偶校验。程序烧录后，串口会输出以下信息：

[.text-center]
--
image::Snipaste_2026-08-04_17-28-56.png[role="thumb"]
--
...
使用 soft i2c 时，不需要在 FSP 中配置引脚和添加 stack，引脚配置和时序均在 src/softiic.c 中完成，一般使用流程如下：
...
// 使用 soft i2c 读写
SoftiicReadMem(&s_iic, ...);
SoftiicWriteMem(&s_iic, ...);
----
