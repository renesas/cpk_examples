...
static fsp_err_t pcdc_write_1m_data(void)
{
	...
	for (i = 0; i < PACKET_CNT; i++) {
		err = R_USB_Write(&g_basic_ctrl,
				test_buf, sizeof(test_buf), USB_CLASS_PCDC);
		if (FSP_SUCCESS != err) {
	...
	sprintf((char *)test_buf, "\r\nPCDC device write %d bytes use %d ms, "
			...
			i * TEST_BUFSZ, ms,
			(((uint32_t)i * TEST_BUFSZ) >> 10) * 1000 / ms);
...
static fsp_err_t pcdc_read_1m_data(void)
{
	...
	while (size < (TEST_BUFSZ * PACKET_CNT)) {	
		err = R_USB_Read(&g_basic_ctrl, test_buf, TEST_BUFSZ, USB_CLASS_PCDC);
		if (err != FSP_SUCCESS) {
