...
static fsp_err_t vee_speed_test(void);
...
static fsp_err_t vee_speed_test(void)
{
	...
	fsp_err_t err;
	uint8_t *addr;
	static uint8_t test_buf[4096];

	APP_PRINT("%s\n", __func__);

	memset(test_buf, 'h', sizeof(test_buf));

	init_cycle_counter(false);

	us = (uint32_t)get_system_us();
	err = vee_write_operation(REC_ID_1, test_buf, sizeof(test_buf));
	us = (uint32_t)get_system_us() - us;
	if (err)
		return err;
	APP_PRINT("vee write 4KB use %u us. Write speed %u Byte/s\n",
	        (uint32_t)us, (uint32_t)(4096 * 1000 / us * 1000));

	err = RM_VEE_FLASH_RecordPtrGet(&g_vee_ctrl, REC_ID_1, &addr, &size);
	if (err) {
		APP_PRINT("get record ptr fail.\n");
		return err;
	}

	us = (uint32_t)get_system_us();
	memcpy(test_buf, addr, 4096);
	us = (uint32_t)get_system_us() - us;

	APP_PRINT("vee read 4KB use %u us. Read speed %u Byte/s\n",
	        (uint32_t)us, (uint32_t)((4096 * 1000) / us * 1000));
