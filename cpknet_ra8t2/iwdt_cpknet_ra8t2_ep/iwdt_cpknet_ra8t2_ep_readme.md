## 1.参考例程概述
该示例项目演示了基于瑞萨 FSP 的瑞萨 RA MCU 上 IWDT 驱动程序的基本功能。
...
#### 1.2.1 gpt 属性配置,选择GPT0, Periodic mode, 周期1s.
详见工程配置。

### 1.3 Stack中添加“Independent Watchdog（r_iwdt）”.
![alt text](images/iwdt0.png)
#### 1.3.1 iwdt 属性配置
![alt text](images/iwdt_config.png)
#### 1.3.2 Stack中添加外部中断0 和 1 “External IRQ（r_icu）”.
![alt text](images/irq0.png)

详细配置参看工程FSP配置。
作为USER KEY 和 MD KEY的按键输入检测。
按下USER KEY启动iwdt，按下MD KEY停止喂狗，iwdt超时MCU复位。
