...
int64_t get_system_ticks(void)
{
    ...
    __PERFC_SAFE {
        ...
        if (lTemp < PERFC.Ticks.lOldTimestamp) {
            /* Missed PMU/systick overflows detected: compensate by adding
             * the appropriate number of overflow periods instead of simply
             * clamping, to avoid permanent time stall.
             */
            int64_t lLoad  = perfc_port_get_system_timer_top() + 1;
            int64_t lDiff  = PERFC.Ticks.lOldTimestamp - lTemp;
            int64_t lMissed = (lDiff + lLoad - 1) / lLoad;
            PERFC.Ticks.lTimestampBase += lMissed * lLoad;
            lTemp = check_systick() + PERFC.Ticks.lTimestampBase;
        }
