...
{coreboardmcu} MCU支持单一3.3V供电，以简化系统设计。{coreboardname}核心板就采用了单电源3.3V设计。
...
{coreboardname}核心板上使用按键SW202控制MD管脚。
按住SW202（MD管脚接地）的同时，按下并释放RESET（SW201），MCU即可进入SCI Boot模式。

image::{core_bd_name}/docs/images/03_hardware/boot_key.png[role="thumb",width=480]

在Boot模式下进行某些操作时（如芯片初始化），需要复位{coreboardmcu} MCU后才能进行下一步，此时需要按照前面进入BOOT的流程再操作一遍。
