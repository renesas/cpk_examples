...
image::Snipaste_2026-08-17_09-37-42.png[role="thumb"]
...
[cols="12,12,20,~,~", options="header"]
|====
| 代码运行  | 数据存储 | D-Cache设置     | Coremark | 说明                                         
| I-TCM | D-TCM | Write-Back    ^| 4.20     | 推荐有时间确定性和高性能要求的代码使用 (如特定的ISR，需要频繁调用的算法等)          
5+|                                             
| MRAM  | SRAM | OFF           ^| 0.59     | 一定要打开D-Cache，否则性能暴跌
| MRAM  | SRAM | Write-Back    ^| 4.22     | 推荐日常使用                  
| MRAM  | SRAM | Write-Through ^| 3.98     | Cache WT会有10%左右的性能损失
5+|                                          
| MRAM  | D-TCM | OFF           ^| 2.99     .3+| D-Cache对D-TCM本身不起作用，但即便使用了D-TCM，也请不要禁用D-Cache。（您程序的数据可能并不全在定义的数据段）
| MRAM  | D-TCM | Write-Back    ^| 4.23       
