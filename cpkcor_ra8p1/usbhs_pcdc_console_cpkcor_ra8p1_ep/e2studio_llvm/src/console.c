...
#define __CONSOLE_DEBUG		1
#endif

#if __CONSOLE_DEBUG
#include "utils/log.h"
#define CONSOLE_LOGD(msg, ...)	LOG_D(__FUNCTION__, msg, ##__VA_ARGS__)
#define CONSOLE_LOGI(msg, ...)	LOG_I(__FUNCTION__, msg, ##__VA_ARGS__)
#define CONSOLE_LOGW(msg, ...)	LOG_W(__FUNCTION__, msg, ##__VA_ARGS__)
#define CONSOLE_LOGE(msg, ...)	LOG_E(__FUNCTION__, msg, ##__VA_ARGS__)
#else
#define CONSOLE_LOGD(msg, ...)
#define CONSOLE_LOGI(msg, ...)
#define CONSOLE_LOGW(msg, ...)
#define CONSOLE_LOGE(msg, ...)
#endif
...
union USB_GenericStatus{
	uint16_t val;
	struct {
		uint16_t apply_read : 1;
		uint16_t apply_write : 1;
		uint16_t attach : 1;
		uint16_t irq_detect : 1;
		uint16_t ready : 1;
...
static union USB_GenericStatus s_usb_generic_status;
static usb_pcdc_linecoding_t s_usb_line_coding;

static uint8_t s_wfifo[CONSOLE_CFG_USB_FIFO_LEN][CONSOLE_CFG_USB_PACK_SIZE] __attribute__((aligned(4)));
static uint32_t s_wfifo_pos;
static uint32_t s_wfifo_each_len[CONSOLE_CFG_USB_FIFO_LEN];

static uint8_t s_rfifo[CONSOLE_CFG_USB_FIFO_LEN][CONSOLE_CFG_USB_PACK_SIZE] __attribute__((aligned(4)));
static uint32_t s_rhead;
static uint32_t s_rtail;
static uint32_t s_rfifo_each_len[CONSOLE_CFG_USB_FIFO_LEN];

static void usbEventCheck(void);
static void usbRead(void);
static void usbWaitWriteDone(void);
static void usbWrite(void);

#endif /* #if CONSOLE_CFG_USE_USB */
...
#elif CONSOLE_CFG_USE_LCD /* #if CONSOLE_CFG_USE_RPMSG */

extern void CONSOLE_LCD_Putchar(char ch);

#endif /* #if CONSOLE_CFG_USE_RPMSG */
...
static int __fputc(char ch, FILE *f)
{
    ...
    while ((ctrl->p_reg->CSR & R_SCI_B0_CSR_TDRE_Msk) == 0) {}
#elif CONSOLE_CFG_USE_USB
    /* FIFO is full, ignore all data */
    if (s_wfifo_pos >= CONSOLE_CFG_USB_FIFO_LEN) {
    	return -1;
    }
    if (s_wfifo_each_len[s_wfifo_pos] >= CONSOLE_CFG_USB_PACK_SIZE) {
    	s_wfifo_pos++;
    	if (s_wfifo_pos >= CONSOLE_CFG_USB_FIFO_LEN) {
    		return -1;
    	}
    }
    s_wfifo[s_wfifo_pos][s_wfifo_each_len[s_wfifo_pos]] = (uint8_t)ch;
    s_wfifo_each_len[s_wfifo_pos]++;
    if (ch == '\n') {
    	s_wfifo_pos++;
    }
...
#else /* #if defined(__ARMCOMPILER_VERSION) this branch is for gcc */
...
static void usbEventCheck(void)
{
	...
	case USB_STATUS_POWERED:
		CONSOLE_LOGD("Powered");
		break;
	case USB_STATUS_DEFAULT:
		CONSOLE_LOGD("Default");
		break;
	case USB_STATUS_ADDRESS:
		CONSOLE_LOGD("Address");
		break;
	case USB_STATUS_CONFIGURED:
		CONSOLE_LOGD("Configured");
		s_usb_generic_status.b.attach = 1;
		s_usb_generic_status.b.ready = 1;
		break;
	case USB_STATUS_SUSPEND:
		CONSOLE_LOGD("Suspend");
		s_usb_generic_status.b.attach = 0;
		s_usb_generic_status.b.ready = 0;
		break;
	...
	case USB_STATUS_REQUEST:
		CONSOLE_LOGD("Request");
		request = usb_event.setup.request_type & USB_BREQUEST;
		if (request == USB_PCDC_SET_LINE_CODING) {
			R_USB_PeriControlDataGet(CONSOLE_CFG_USB_INSTANCE.p_ctrl, (uint8_t *)&s_usb_line_coding, 0x07);
		}
		else if (request == USB_PCDC_GET_LINE_CODING) {
			R_USB_PeriControlDataSet(CONSOLE_CFG_USB_INSTANCE.p_ctrl, (uint8_t *)&s_usb_line_coding, 0x07);
		}
		else if (request == USB_PCDC_SET_CONTROL_LINE_STATE) {
			R_USB_PeriControlStatusSet(CONSOLE_CFG_USB_INSTANCE.p_ctrl, USB_SETUP_STATUS_ACK);
		}
		break;
	case USB_STATUS_REQUEST_COMPLETE:
		CONSOLE_LOGD("Request Complete");
		break;
	case USB_STATUS_READ_COMPLETE:
		CONSOLE_LOGD("Read Complete");
		s_usb_generic_status.b.read_done = 1;
		s_usb_generic_status.b.apply_read = 0;
		s_rfifo_each_len[s_rtail] = usb_event.data_size;
		s_rtail++;
		if (s_rtail >= CONSOLE_CFG_USB_FIFO_LEN) {
			s_rtail = 0;
		}
		if (s_rtail == s_rhead) {
			s_rhead++;
		}
		if (s_rhead >= CONSOLE_CFG_USB_FIFO_LEN) {
			s_rhead = 0;
		}
		break;
	case USB_STATUS_WRITE_COMPLETE:
		CONSOLE_LOGD("Write Complete");
		s_usb_generic_status.b.write_done = 1;
		break;
	case USB_STATUS_BC:
		CONSOLE_LOGD("BC");
		break;
	case USB_STATUS_OVERCURRENT:
		CONSOLE_LOGD("Overcurrent");
		break;
...
static void usbRead(void)
{
	...
	if (s_usb_generic_status.b.apply_read && (s_usb_generic_status.b.read_done == 0)) {
		return;
	}

	err = R_USB_Read(CONSOLE_CFG_USB_INSTANCE.p_ctrl, &s_rfifo[s_rtail][0], CONSOLE_CFG_USB_PACK_SIZE, USB_CLASS_PCDC);
	if (err) {
		CONSOLE_LOGE("USB Read failed: %" PRIu32, err);
		return;
	}

	s_usb_generic_status.b.apply_read = 1;
	s_usb_generic_status.b.read_done = 0;
}
...
static void usbWaitWriteDone(void)
{
	while (1) {
		usbEventCheck();
		if (s_usb_generic_status.b.attach == 0) {
			break;
		}
		if (s_usb_generic_status.b.ready == 0) {
			break;
		}
		if (s_usb_generic_status.b.write_done) {
			break;
		}
	}
}
...
static void usbWrite(void)
{
	...
	/* When PC close CDC and USB detach */
	if (s_usb_generic_status.b.apply_write) {
		/* NOTE: Don't close, otherwise USB_Read() will always failed, code: 1500 */
	#if 0
		err = R_USB_Close(CONSOLE_CFG_USB_INSTANCE.p_ctrl);
		if (err) {
			CONSOLE_LOGE("USB Close failed: %" PRIu32, err);
			/* Not clear apply_write, so it will retry when next exec to here */
			return;
		}
		err = R_USB_Open(CONSOLE_CFG_USB_INSTANCE.p_ctrl, CONSOLE_CFG_USB_INSTANCE.p_ctrl);
		if (err) {
			CONSOLE_LOGE("USB Open failed: %" PRIu32, err);
			/* Not clear apply_write, so it will retry when next exec to here */
			return;
		}
	#endif
		s_usb_generic_status.b.apply_write = 0;
		return;
	}

	if (s_wfifo_pos) {
		s_usb_generic_status.b.apply_write = 1;
		s_usb_generic_status.b.write_done = 0;
		err = R_USB_Write(CONSOLE_CFG_USB_INSTANCE.p_ctrl, &s_wfifo[s_wfifo_pos - 1][0], s_wfifo_each_len[s_wfifo_pos - 1], USB_CLASS_PCDC);
		switch (err) {
		case FSP_ERR_USB_BUSY:
			CONSOLE_LOGW("USB Busy");
			s_usb_generic_status.b.apply_write = 0;
			break;
		case FSP_ERR_USB_FAILED:
			CONSOLE_LOGE("USB Failed");
			s_usb_generic_status.b.apply_write = 0;
			break;
		case FSP_SUCCESS:
			usbWaitWriteDone();
			if (s_usb_generic_status.b.write_done) {
				s_usb_generic_status.b.apply_write = 0;
				s_wfifo_each_len[s_wfifo_pos - 1] = 0;
				s_wfifo_pos--;
			}
			else {
				CONSOLE_LOGE("Data not wrie done");
				if (s_usb_generic_status.b.attach == 0) {
					CONSOLE_LOGI("USB deattach");
				}
			}
			break;
		default:
			CONSOLE_LOGW("Uncheck return: %" PRIu32, err);
			break;
		}
	}
}
...
int CONSOLE_HasData(void)
{
	return s_rhead != s_rtail ? 1 : 0;
}

void CONSOLE_Init(void)
{
	s_usb_generic_status.val = 0;
	s_wfifo_pos = 0;
	s_rhead = 0;
	s_rtail = 0;
	memset(s_wfifo, 0, sizeof(s_wfifo));
	memset(s_wfifo_each_len, 0, sizeof(s_wfifo_each_len));
	memset(s_rfifo, 0, sizeof(s_rfifo));
	memset(s_rfifo_each_len, 0, sizeof(s_rfifo_each_len));

	R_USB_Open(CONSOLE_CFG_USB_INSTANCE.p_ctrl, CONSOLE_CFG_USB_INSTANCE.p_cfg);
}

unsigned CONSOLE_Read(void* buffer, unsigned size)
{
	uint32_t i, j;
	uint32_t read_num, copied;
	...
	if (s_rhead < s_rtail) {
		for (i = s_rhead; i < s_rtail; i++) {
			rx_length += s_rfifo_each_len[i];
		}
		read_num = rx_length > size ? size : rx_length;
		copied = read_num;
		...
		i = s_rhead;
		while (read_num) {
			if (read_num < s_rfifo_each_len[i]) {
				memcpy(p8, &s_rfifo[i][0], read_num);
				/* Move remain data to its start position */
				for (j = 0; j < (s_rfifo_each_len[i] - read_num); j++) {
					s_rfifo[i][j] = s_rfifo[i][j + read_num];
				}
				s_rfifo_each_len[i] -= read_num;
				read_num = 0;
			}
			else {
				memcpy(p8, &s_rfifo[i][0], s_rfifo_each_len[i]);
				p8 = &p8[s_rfifo_each_len[i]];
				s_rhead++;
				read_num -= s_rfifo_each_len[i];
				s_rfifo_each_len[i] = 0;
			}
			i++;
		}
	}
	else {
		for (i = s_rhead; i < CONSOLE_CFG_USB_FIFO_LEN; i++) {
			rx_length += s_rfifo_each_len[i];
		}
		for (i = 0; i < s_rtail; i++) {
			rx_length += s_rfifo_each_len[i];
...
void CONSOLE_Task(void)
{
	...
	if (s_usb_generic_status.b.ready) {
		usbWrite();
		usbRead();
	}
}
...
#else /* #if CONSOLE_CFG_USE_RTT this branch is for CONSOLE_CFG_USE_CUSTOM */
...
/* Normally, it's for CONSOLE_CFG_USE_RPMSG and when CPU is CPU1 */
#if I_AM_PRINTER
void CONSOLE_IPCIrqHandle(void)
{
