...
#define ST7796U_4SPI_EN_CACHE           0
#endif
...
uint32_t LCD_PortDrawBitmap(LCD_Device *device, uint16_t x, uint16_t y, uint16_t length, uint16_t width, const uint8_t *bitmap)
{
	...
	for (i = 0; i < width; i++) {
		for (j = 0; j < length; j++) {
			color = p16_bitmap[i * src_length + j];
			r = (color >> 11) & 0x1F;
			b = color & 0x1F;
			color |= r;
			color |= (b << 11);
			p16_cache[index + j] = bswap_16(color);
		}
		index += device->width;
	}
    ...
    R_SCI_B_SPI_Write(g_sci_spi4.p_ctrl, &p16_cache[y * device->width], width * device->width * 2, SPI_BIT_WIDTH_8_BITS);
#else
...
uint32_t LCD_PortFillRectangle(LCD_Device *device, uint16_t x, uint16_t y, uint16_t length, uint16_t width, uint32_t color)
{
    ...
    R_BSP_PinWrite(ST7796U_PIN_SPI_CS, BSP_IO_LEVEL_LOW);
    if (device->color_format == COLOR_FORMAT_RGB565) {
        R_SCI_B_SPI_Write(g_sci_spi4.p_ctrl, &p16_cache[y * device->width], row_num * device->width * 2, SPI_BIT_WIDTH_8_BITS);
    }
    else {
        R_SCI_B_SPI_Write(g_sci_spi4.p_ctrl, &p32_cache[y * device->width], row_num * device->width * 4, SPI_BIT_WIDTH_8_BITS);
    }
...
uint32_t LCD_PortFillScreen(LCD_Device *device, uint32_t color)
{
    ...
    if (device->color_format == COLOR_FORMAT_RGB565) {
        ...
        R_BSP_PinWrite(ST7796U_PIN_DCX, BSP_IO_LEVEL_HIGH);
        R_BSP_PinWrite(ST7796U_PIN_SPI_CS, BSP_IO_LEVEL_LOW);
        R_SCI_B_SPI_Write(g_sci_spi4.p_ctrl, p_cache, ST7796U_CACHE_SIZE, SPI_BIT_WIDTH_8_BITS);
    #else
	#if ST7796U_SPI_HARD_CS == 0
