...
VOID ra_get_usb_camera_configurations(NX_TCP_SOCKET *tcp_socket, UX_HOST_CLASS_VIDEO *host_class_video);

static void ra_select_desired_configuration(void);
...
#define PIC_WIDTH               1280//640//(320)
#define PIC_HEIGHT              720//480//(240)
#define PIC_FRAME_INTERVAL      (1000000)//(1000000)//((666666)//(1000000)//(333333)//(1000000)//(333333)


streamer_data_t     streamer_data BSP_PLACE_IN_SECTION(".sdram");
streamer_data_t     *pstreamer_data;
...
void ra_usbx_host_uvc_camera_entry(void)
{
   ...
   pstreamer_data = &streamer_data;
   memset(pstreamer_data->http_jpg_data_left,0,sizeof(pstreamer_data->http_jpg_data_left));
   memset(pstreamer_data->http_jpg_data_right,0,sizeof(pstreamer_data->http_jpg_data_right));
   pstreamer_data->index = 0;

   pstreamer_data->len[0] = 0;
   pstreamer_data->len[1] = 0;
    R_BSP_PinWrite(BSP_IO_PORT_10_PIN_01, 0);
    while (1)
    {
        tx_event_flags_get (&g_usb_camera_device_connected_event_flags0, USB_CAMERA_DEVICE_CONNECT_FLAG, TX_OR, &event_fvlaue, TX_WAIT_FOREVER);
            ...
            while (1)
            {
                ...
                if (0 != g_actual_tra_req_len[completed_transfer_index])
                {
                    ...
                    if (0 != jpeg_stream.size)
                    {
                        ...
                        {   
                            memcpy(&pstreamer_data->http_jpg_data_left[cnt],jpeg_stream.p_pointer,jpeg_stream.size);

                        }
                        else
                        {
                            memcpy(&pstreamer_data->http_jpg_data_right[cnt],jpeg_stream.p_pointer,jpeg_stream.size);
                        ...
                        if (is_eof)
                        {
                            APP_PRINT("frame_bytes_sent: 0x%x packetno: 0x%x\n", frame_bytes_sent,packetno);

                            pstreamer_data->len[pstreamer_data->index]=cnt;
                            tx_semaphore_put(&g_http_data_ready);

                            if(pstreamer_data->index==0)
                            {
                                pstreamer_data->index = 1;
                            }
                            else
                            {
                                pstreamer_data->index = 0;
...
VOID ra_ux_host_uvc_transfer_completion_callback(UX_TRANSFER *transfer_request)
{
    ...
    //APP_PRINT("\r\n Host uvc transfer completed = %d\r\n", g_actual_tra_req_len[g_callback_index]);
...
static void set_video_parameters_start_video(void)
{
    ...
    while(true)
    {
        desired_format = UX_HOST_CLASS_VIDEO_VS_FORMAT_MJPEG;
        desired_resolution_x = PIC_WIDTH;//320;
        desired_resolution_y = PIC_HEIGHT;//240;
        desired_frame_interval = PIC_FRAME_INTERVAL;//333333;
        APP_PRINT("\r\n video parameters:\r\n Desired_format = %d.\r\n Resolution X = %d.\r\n Resolution Y = %d. \r\n Frame interval = %d. \r\n",
