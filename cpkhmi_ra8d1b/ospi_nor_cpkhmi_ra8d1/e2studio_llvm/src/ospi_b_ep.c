...
#define OSPI_TEST_START	(0x90000000)
#define OSPI_TEST_SIZE	(0x1000000)
#define PAGE_SIZE	4096

static __attribute__((aligned(8))) uint8_t g_test_data[4096];
...
fsp_err_t ospi_b_set_protocol_to_opi (void)
{
    ...
    if(SPI_FLASH_PROTOCOL_8D_8D_8D == g_ospi_b_ctrl.spi_protocol) {
        ...
        transfer.command = 0x85;
        transfer.command_length = 1;
        transfer.data = 0;
        transfer.data_length = 1;
        transfer.dummy_cycles = 8;
        R_OSPI_B_DirectTransfer(g_ospi_b.p_ctrl, &transfer, SPI_FLASH_DIRECT_TRANSFER_DIR_READ);
        APP_PRINT("VCR-IOC: 0x%02X\n", transfer.data);
...
static fsp_err_t ospi_erase_chip(void)
{
	fsp_err_t err;
	uint32_t ms;
	...
	err = ospi_b_wait_operation(0x40000000);
	if (err) {
	    APP_PRINT("erase chip fail.\n");
	    return err;
	}

	ms = get_system_ms() - ms;
	APP_PRINT("Erase chip use %u ms\n", ms);

	return 0;
}

static void do_benchmark_test(void)
{
    uint32_t i, ms;
    uint32_t value;
    fsp_err_t err;

	unsigned char *base = (unsigned char *)OSPI_TEST_START;
	volatile uint32_t *addr = (uint32_t *)OSPI_TEST_START;
	...
	APP_PRINT("start OSPI write speed test.\r\n");
	ms = (uint32_t)get_system_ms();
	for (i = 0; i < OSPI_TEST_SIZE / sizeof(g_test_data); i++) {
		err = R_OSPI_B_Write(&g_ospi_b_ctrl, g_test_data, base, PAGE_SIZE);
		if (err) {
			APP_PRINT("%s: write operation fail\n");
			return;
		}

		err = ospi_b_wait_operation(1000000);
		if (err) {
			APP_PRINT("%s: write operation fail timeout\n");
			return;
		}
	...
	APP_PRINT("\r\nWrite %u bytes use %u ms, OSPI write speed is %u KB/s\r\n", OSPI_TEST_SIZE, ms, OSPI_TEST_SIZE / 1024 * 1000 / ms);

	APP_PRINT("start OSPI read speed test.\r\n");
	ms = (uint32_t)get_system_ms();
	for (i = 0; i < OSPI_TEST_SIZE / sizeof(uint32_t); i++) {
		value = *addr;
		addr++;
	}
	ms = (uint32_t)get_system_ms() - ms;
	APP_PRINT("\r\nRead %u bytes use %u ms, OSPI read speed is %u KB/s\r\n", OSPI_TEST_SIZE, ms, OSPI_TEST_SIZE / 1024 * 1000 / ms);
}
    ...
    while(true)
    {
        if(APP_CHECK_DATA)
        {
            ...
            switch(rtt_read[RTT_CHECK_INDEX])
            {
                ...
                case RTT_SELECT_SPEED_TEST_CHAR:
                    APP_PRINT("\r\nStart OSPI read/write speed test.\r\n");
                    do_benchmark_test();
                    break;
...
static fsp_err_t ospi_b_setup_calibrate_data(void)
{
    ...
    if (RESET_VALUE != memcmp((uint8_t *)OSPI_B_APP_ADDRESS(OSPI_B_SECTOR_THREE), &g_autocalibration_data, sizeof(g_autocalibration_data)))
    {
        /* Erase the flash sector that stores auto-calibration data */
        err = R_OSPI_B_Erase(&g_ospi_b_ctrl, (uint8_t *)OSPI_B_APP_ADDRESS(OSPI_B_SECTOR_THREE), OSPI_B_SECTOR_SIZE_4K);
        APP_ERR_RETURN(err, "R_OSPI_B_Erase API FAILED \r\n");
        ...
        err = R_OSPI_B_Write(&g_ospi_b_ctrl, (uint8_t *)&g_autocalibration_data, (uint8_t *)OSPI_B_APP_ADDRESS(OSPI_B_SECTOR_THREE), sizeof(g_autocalibration_data));
        APP_ERR_RETURN(err, "R_OSPI_B_Write API FAILED \r\n");
